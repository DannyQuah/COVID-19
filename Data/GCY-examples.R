#! R
# @(#) GCY-examples.R
# Last-edited: Fri 2020.07.31.2032  -- Danny Quah (me@DannyQuah.com)
# ----------------------------------------------------------------
# Revision History:
#  % Fri 2020.07.31.1506  -- Danny Quah (me@DannyQuah.com)
#    First draft: R script from Guangchuang Yu github // example.R
# ----------------------------------------------------------------
library(tidyverse)
library(lubridate)
...
p <- ggplot(dd, aes(country, cum_confirm, fill=cum_confirm)) +
    ...
    geom_shadowtext(aes(label=paste(country, cum_confirm, sep="\n"),
                  y = y, angle = angle, vjust = vjust),
            data=function(d) d[d$cum_confirm > label_cut,],
            size=3, colour = "white", bg.colour="black", fontface="bold")  +
